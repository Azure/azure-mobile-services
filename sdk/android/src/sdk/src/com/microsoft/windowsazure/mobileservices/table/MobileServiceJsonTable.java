...
public final class MobileServiceJsonTable extends MobileServiceTableBase<JsonElement> {
	 ...
	 *            Callback to invoke when the operation is completed
	 */
	public void execute(final MobileServiceQuery<?> query, final TableQueryCallback<JsonElement> callback) {
	...
	public void lookUp(Object id, List<Pair<String, String>> parameters, final TableJsonOperationCallback callback) {
		...
		ListenableFuture<JsonElement> lookUpFuture = lookUp(id, parameters);

		Futures.addCallback(lookUpFuture, new FutureCallback<JsonElement>() {
			@Override
			public void onFailure(Throwable exception) {
				if (exception instanceof Exception) {
			...
			@Override
			public void onSuccess(JsonElement result) {
				callback.onCompleted((JsonObject)result, null, null);
			}
		});
	}
