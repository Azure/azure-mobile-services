...
public class MobileServicePush {
    ...
    private ListenableFuture<Void> createOrUpdateInstallation(String pnsHandle, JsonElement templates)
    {
        ...
        requestHeaders.add(new Pair<>("Content-Type", MobileServiceConnection.JSON_CONTENTTYPE));

        ListenableFuture<ServiceFilterResponse> serviceFilterFuture = mHttpClient.request(path, installation.toString(), "PUT", requestHeaders, null, EnumSet.noneOf(MobileServiceFeatures.class));
