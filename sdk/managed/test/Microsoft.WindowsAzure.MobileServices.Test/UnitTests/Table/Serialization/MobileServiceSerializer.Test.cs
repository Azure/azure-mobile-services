...
namespace Microsoft.WindowsAzure.MobileServices.Test
{
    ...
    public class MobileServiceSerializerTests : TestBase
    {
        ...
        {
            List<Tuple<object, object>> testCases = new List<Tuple<object, object>>() {
                new Tuple<object, object>(new BoolType() { Bool = true }, 5),
                new Tuple<object, object>(new BoolType() { Bool = false, Id = 15 }, 5),
                new Tuple<object, object>(JToken.Parse("{\"id\":0}"), 5),
                new Tuple<object, object>(JToken.Parse("{\"id\":25}"), 5),
                new Tuple<object, object>(JToken.Parse("{\"id\":0.0}"), 5),
                new Tuple<object, object>(JToken.Parse("{\"id\":25.0}"), 5),
                new Tuple<object, object>(JToken.Parse("{}"), 5),
                new Tuple<object, object>(JToken.Parse("{\"id\":null}"), 5),
            };
            ...
            foreach (var testCase in testCases)
            {
                ...
                DefaultSerializer.SetId(input, 5);
                var actual = DefaultSerializer.GetId(input);
        ...
        public void ClearId()
        {
            List<Tuple<object, object>> testCases = new List<Tuple<object, object>>() {
                ...
                new Tuple<object, object>(JToken.Parse("{\"id\":0}"), null),
                new Tuple<object, object>(JToken.Parse("{\"id\":25}"), null),
                new Tuple<object, object>(JToken.Parse("{\"id\":0.0}"), null),
                new Tuple<object, object>(JToken.Parse("{\"id\":25.0}"), null),
                new Tuple<object, object>(JToken.Parse("{}"), null),
                new Tuple<object, object>(JToken.Parse("{\"id\":null}"), null),
            };
        ...
        {
            List<Tuple<string, string>> testCases = new List<Tuple<string, string>>() {
                new Tuple<string, string>("{\"ULong\":18446744073709551615}","JSON integer 18446744073709551615 is too large or small for an Int64. Path 'ULong', line 1, position 29."), // This fails because JSON.NET will try to create a long instead of a ulong    
