...
namespace Microsoft.WindowsAzure.MobileServices
{
    ...
    internal partial class MobileServiceTable<T> : MobileServiceTable, IMobileServiceTable<T>
    {
        ...
        internal static async Task<IEnumerable<U>> EvaluateQueryAsync<U>(IMobileServiceTable<T> table, MobileServiceTableQueryDescription query)
        {
            ...
            JToken response = await table.ReadAsync(odata);

            // Parse the results
            long totalCount;
            JToken values = MobileServiceTable<T>.GetResponseSequence(response, out totalCount);
        ...
        public async Task<IEnumerable<T>> ReadAsync()
        {
            ...
            string odata = null;
            JToken response = await this.ReadAsync(odata);
        ...
        internal static JArray GetResponseSequence(JToken response, out long totalCount)
        {
            ...
            JArray values = response.AsArray();
            if (values == null)
            {
