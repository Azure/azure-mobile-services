...
public class UpdateDeleteTests extends TestGroup {
	...
	public UpdateDeleteTests() {
		...
		//With Callbacks
		this.addTest(createTypedUpdateWithCallbackTest("With Callback - Update typed item", new RoundTripTableElement(rndGen), new RoundTripTableElement(rndGen), true, null));
		this.addTest(createUntypedUpdateWithCallbackTest("With Callback - Update untyped item, setting values to null", parser.parse(toInsertJsonString).getAsJsonObject(), cloneJson(toUpdate), true,
				null));
	...
	private TestCase createDeleteWithCallbackTest(String name, final boolean typed, final boolean useFakeId, final boolean includeId, Class<?> expectedExceptionClass) {
		TestCase testCase = new TestCase() {
