...
public class QueryTests extends TestGroup {
	...
	public QueryTests() {
		...
		try {
			...
			this.addTest(createQueryTest("Compound statement - OR of ANDs - Movies from the 30s and 50s", field("year").ge(1930).and().field("year").lt(1940)
					.or(field("year").ge(1950).and().field("year").lt(1960)), new SimpleMovieFilter() {
			...
			this.addTest(createQueryTest("Division, equal and different - Movies from the year 2000 with rating other than R", query(field("year").div(1000))
			...
			this.addTest(createQueryTest("Addition, subtraction, relational, AND - Movies from the 1980s which last less than 2 hours", field("year").sub(1900)
					.ge(80).and().field("year").add(10).lt(2000).and().field("duration").lt(120), new SimpleMovieFilter() {
			...
			this.addTest(createQueryTest("Date: Greater than, less than - Movies with release date in the 80s",
					field("releaseDate").ge(Util.getUTCDate(1980, 1, 1)).and().field("releaseDate").le(Util.getUTCDate(1989, 1, 1, 23, 59, 59)),
					new SimpleMovieFilter() {
			...
			this.addTest(createQueryTest("Bool: not equal to false - Best picture winners after 2000", field("bestPictureWinner").ne(false).and().field("year")
					.ge(2000), new SimpleMovieFilter() {
			...
			this.addTest(createQueryTest(
					"Select multiple fields - List of movies from the 2000's",
					field("year").ge(2000),
					new SimpleMovieFilter() {
			...
			this.addTest(createQueryTest("(Neg) Very large top value", field("year").gt(2000), null, 1001, null, null, null, false,
			...
			// With Callback
			this.addTest(createQueryWithCallbackTest(
					"With Callback - Addition, subtraction, relational, AND - Movies from the 1980s which last less than 2 hours",
			...
			this.addTest(createQueryWithCallbackTest("With Callback - Date (year): Movies whose year is different than its release year", year("releaseDate")
					.ne().field("year"), new SimpleMovieFilter() {
			...
			this.addTest(createQueryWithCallbackTest("With Callback - Bool: not equal to false - Best picture winners after 2000", field("bestPictureWinner")
					.ne(false).and().field("year").ge(2000), new SimpleMovieFilter() {
			...
			this.addTest(createQueryWithCallbackTest(
					"With Callback - Select multiple fields - List of movies from the 2000's",
					field("year").ge(2000),
					new SimpleMovieFilter() {
