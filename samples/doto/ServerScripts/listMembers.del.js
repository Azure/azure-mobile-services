...
function del(id, user, context) {
    ...
    listMembers.where({ id: id }).read({
        success: function (results) {
            if (results.length === 0) {
                ...
                context.respond(404, 'Cannot delete a membership that does not exist');
                return;
            }

            var membership = results[0];
            if (membership.userId !== user.userId) {
                context.respond(400, 'You cannot remove another user from a list');
                return;
            }
    ...
    function deleteItems(listId) {
        // This user is the last user of that list, so we need to
        // do some cleanup.
